	...
	for (int i=0; i < dpcComplex.cols; i++){
	   for (int j=0; j < dpcComplex.rows; j++){
	      for (int k=0; k < dpcList.size(); k++)
	      {
	        ...
	        Hi_sum += std::complex<double>(0,-1*transferFunctionList.at(k).at<double>(j,i));
	        I_sum += std::complex<double>(dpcFtRealList.at(k).at<double>(i,j),dpcFtImagList.at(k).at<double>(i,j));
	      ...
	      H_sum = std::complex<double>(0,0);
	      Hi_sum = std::complex<double>(0,0);
	      I_sum = std::complex<double>(0,0);
	   }
	}
	double c2 = clock();
